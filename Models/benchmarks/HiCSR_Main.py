...
def load_training_data(input_dir, ratio):
    ...
    hr_valid = np.moveaxis(hr_valid, -1, 1)
    lr_valid = np.moveaxis(lr_valid, -1, 1)

    # Normalize outliers
    # hr_train_cutoff = np.percentile(np.vstack([hr_train, hr_valid, lr_train, lr_valid]), 99.9)
    ...
    scale_val = 1#np.max(np.vstack([hr_train, hr_valid, lr_train, lr_valid]))
    #Scale values
    hr_train /= scale_val
    lr_train /= scale_val
    hr_valid /= scale_val
    lr_valid /= scale_val
    return lr_train, hr_train, lr_valid, hr_valid, float(scale_val)

def load_test_data(input_dir, ratio, scale):
...
def train_the_model(args) -> None:
    ...
    lr_train, hr_train, lr_valid, hr_valid, scale_val = load_training_data(args.input, args.ratio)
    args.scale_val = scale_val
    ...
    train_data = torch.tensor(hr_train, dtype=torch.float)
    train_target = torch.tensor(lr_train, dtype=torch.float)
    train_set = TensorDataset(train_data, train_target)


    valid_data = torch.tensor(hr_valid, dtype=torch.float)
    valid_target = torch.tensor(lr_valid, dtype=torch.float)
...
def impute_the_target(args):
    if os.path.exists(f"{args.save_dir}/commandline_args.json"):
        ...
        scale_val = training_args["scale_val"]
    print(f"Scale val is: {scale_val}")
    model_path = os.path.join(args.save_dir, "models", f'best_model.pytorch')
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    # load network
    netG = Generator(num_channels=1).to(device)
    netG.load_state_dict(torch.load(model_path, map_location=torch.device(device)))
    netG.eval()
    BATCH_SIZE = args.batch_size_per_gpu 

    with torch.no_grad():
        lr_test_dict = load_test_data(args.input, args.ratio, scale_val)
        for keey, vall in lr_test_dict.items():
            ...
            for lr in tqdm(test_loader, desc='HiCSR Predicting: '):
                lr = lr[0].to(device)
                out = netG(lr).cpu().detach().numpy()
                
                result_data.append(out)
            result_data = np.concatenate(result_data, axis=0) * scale_val
            result_data = np.moveaxis(result_data, 1, -1).astype(np.float32) 
...
def main():
    deciding_args_parser = argparse.ArgumentParser(description='HiCSR.', add_help=False)
